...
export default function AppPage() {
  ...
  // Define apps based on user role
  const getAvailableApps = () => {
    ...
    if (mode === "admin" && userRole === "admin") {
      ...
      return allApps.filter((app) => app.id === "prep")
    }
  }
  ...
  const keyboardShortcuts = [
    { key: "Alt + 0", description: "Ir a Dashboard", condition: mode === "admin" && userRole === "admin" },
    { key: "Alt + 1", description: "Ir a Administración", condition: mode === "admin" && userRole === "admin" },
    { key: "Alt + 2", description: "Ir a Preparación", condition: true },
    { key: "Alt + R", description: "Refrescar aplicación actual", condition: true },
    { key: "Alt + F", description: "Pantalla completa", condition: true },
    { key: "Alt + T", description: "Cambiar tema claro/oscuro", condition: true },
    { key: "Alt + S", description: "Abrir configuración", condition: true },
    { key: "Alt + H", description: "Mostrar atajos de teclado", condition: true },
    { key: "Esc", description: "Salir de pantalla completa", condition: true },
    { key: "Flecha izquierda", description: "Aplicación anterior", condition: apps.length > 1 },
    { key: "Flecha derecha", description: "Aplicación siguiente", condition: apps.length > 1 },
  ].filter((shortcut) => shortcut.condition)

  // Check authentication
  useEffect(() => {
    if (mounted && !isAuthenticated && mode === "admin") {
      router.push("/login")
    }
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.altKey) {
        switch (e.key) {
          case "0":
            if (apps.some((app) => app.id === "dashboard")) {
              switchToApp("dashboard")
            }
            break
          case "1":
            if (apps.some((app) => app.id === "admin")) {
              switchToApp("admin")
            }
            break
          case "2":
            if (apps.some((app) => app.id === "prep")) {
              switchToApp("prep")
            }
            break
          case "r":
  ...
  }, [resolvedTheme, setTheme, activeTab, apps])
  ...
  useEffect(() => {
    if (mounted && apps.length > 0) {
      // Set default tab based on available apps
      setActiveTab(apps[0].id)
    }
  }, [mounted, apps])
                ...
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <CardDescription className="text-xs sm:text-sm hidden sm:block">
                      {mode === "admin" ? "Panel de administración" : "Sistema de preparación de pedidos"}
                          ...
                          {accentColors.map((color) => (
                            <div key={color.value} className="flex items-center space-x-2">
                              ...
                              >
                                ...
                                {accentColor === color.value && <span className="text-accent">✓</span>}
                              </Label>
                            </div>
                          ))}
                        </RadioGroup>
